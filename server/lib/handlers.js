...
handlers._users.put = function(data,callback){
    ...
    if (phone){
        //Error if nor=thing is sent to update
                    ...
                    if (!err && userData){
                        _data.delete("users", phone, function (err) {
                            if (!err){
                                        ...
                                        _data.delete("checks", checkId, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete){
                                                if (!deletionErrors){
                                                    ...
                                                    callback(500, {'Error': 'Errors encountered while attempting to delete all of the user\'s checks. All checks may not deleted from the system successfuly'})
                                                }
                                            }
                                        })
                                    })
                                ...
                                callback(500, {"Error": "Could not the specified user"})
                            }
                        })
                    }else{
...
handlers._tokens.post = function(data, callback){
    ...
    if (phone && password){
        //Look up who matches that phone number
        _data.read("users", phone, function (err, userData) {
            if (!err){
                //Hash the send password, and compared to the password stored in the user object
                var hashedPassword = helpers.hash(password)
                if (hashedPassword == userData.hashedPassword){
                    //If valid, create a new token with a random name. Set the expiration date 1hour in the future
            ...
            if (!err && data){
                _data.delete("tokens", id, function (err) {
                    if (!err){
                        ...
                        callback(500, {"Error": "Could not the specified token"})
                    }
                })
            }else{
                                ...
                                if (!err){
                                    ...
                                    _data.update('users', userPhone,userData, function (err) {
                                        if (!err){
                                            ...
                                            callback(500, {'Error': 'Could not the update the user with the new check'})
                                        }
                                    })
                                }else{
    ...
    if (id){
        ...
        _data.read('checks',id,function (err,checkData) {
            if (!err && checkData){
                ...
                callback(400, {'Error': 'The specified check ID daes not exist'})
            }
        })
    }else{
        callback(400, {"Error": "Missing required field"})
